        ...
        <div class="Ma_right">
            ...
            <div>
                <!--每一小块内容-->
	    				...
	    				<c:forEach items="${themeList }" var="route">
	    				<div>
				...
				<div class="cont_botm_nav clearfix">
					<div>
							<form action="<%=basePath%>/route/getRouteByTheme.do?route_themetype=0" method="post">
						<p>
						     <c:if test="${pageno<1 }">
								    	<li style="display: none;"><a>上一页</a></li>
							 </c:if>
							 <c:if test="${pageno>1 }">
								   <a href="<%=basePath %>/route/getRouteByTheme.do?route_themetype=0&pageno=${pageno-1 }">上一页</a>
							 </c:if>       
						     <c:forEach var="i" begin="1" step="1" end="${themePage }">
								    <a href="<%=basePath%>/route/getRouteByTheme.do?route_themetype=0&pageno=${i}">${i }</a>
							 </c:forEach>  
							   
							 <c:if test="${pageno<themePage }">
						        <a href="<%=basePath%>/route/getRouteByTheme.do?route_themetype=0&pageno=${pageno+1}"><span>下一页</span></a>		    
					    	 </c:if>
					    	 <c:if test="${pageno>themePage }">
							 	<li style="display: none;"><a>下一页</a></li>
							 </c:if> 
					    	<span>到</span><input type="text" class="stage_page_in" name="pageno"><span>页</span>
					    		<input type="submit"  class="stage_page_su" value="确定">
					    </p>
					    	</form>
					</div>
				</div>
