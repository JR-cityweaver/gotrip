        ...
        <div class="Ma_right">
            ...
            <div>
                <!--每一小块内容-->
	    				...
	    				<c:forEach items="${hotList }" var="route">
	    				<div>
				...
				<div class="cont_botm_nav clearfix">
					<div>
							<form action="<%=basePath%>/route/hot.do" method="post">
						<p>
						     <c:if test="${pageno<1 }">
								    	<li style="display: none;"><a>上一页</a></li>
							 </c:if>
							 <c:if test="${pageno>1 }">
								   <a href="<%=basePath %>/route/hot.do?pageno=${pageno-1 }">上一页</a>
							 </c:if>       
						     <c:forEach var="i" begin="1" step="1" end="${totalPage }">
								    <a href="<%=basePath%>/route/hot.do?pageno=${i}">${i }</a>
							 </c:forEach>  
							   
							 <c:if test="${pageno<totalPage }">
						        <a href="<%=basePath%>/route/hot.do?pageno=${pageno+1}"><span>下一页</span></a>		    
					    	 </c:if>
					    	 <c:if test="${pageno>totalPage }">
